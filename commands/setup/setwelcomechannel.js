...
exports.run = async (bot, message, args) => {
 ...
 if (!message.member.hasPermission("MANAGE_GUILD")) {
  ...
  {
    if (args[0] === "off") {
    let chn = await db.delete(`${message.guild.id}_welcomechannel`); 
      return message.lineReplyNoMention(     
        new Discord.MessageEmbed()
        .setDescription(`You turn off the welcome channel`)
        .setColor("#00ff11")   
      ) 
  ...
  return message.lineReplyNoMention(     
        new Discord.MessageEmbed()
        .setDescription(`<:pleux_success:887552715247452210> | Successfully setup welcome message send place\n${bot.config.prefix}setwelcomechannel off (turn off welcome Channel)\n if the command has been run and has not shown the channel text, please type *${bot.config.prefix}setwelcomesystem*`)
        .setColor("#00ff11")   
      ) 
